...
import (
	...
	log "hotwave/logger"
	"hotwave/servers/account/cache"
	"hotwave/servers/account/config"
	"hotwave/servers/account/database"
...
func (h *Handler) LoginWithPasswd(ctx context.Context, in *proto.LoginWithPasswdRequest) (*proto.LoginWithPasswdResponse, error) {
	...
	assess, err := rawToekn.SignedString(jwtKey)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) UserInfo(ctx context.Context, in *proto.UserInfoRequest) (*proto.UserInfoResponse, error) {
	claims, err := VerifyAdminTokenWithClaims(in.AccessToken)
	if err != nil {
		return nil, err
	}
...
func VerifyAdminTokenWithClaims(tokenRaw string) (*jwt.RegisteredClaims, error) {
	claims := &jwt.RegisteredClaims{}
